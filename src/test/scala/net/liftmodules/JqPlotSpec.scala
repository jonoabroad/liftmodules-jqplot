    ...
    class JqPlotSpec extends Specification {
      ...
      "JqPlot" should {
        " correctly caculate plugins to add as Nil. " in {
          val jqp = new JqPlot(0, 0, "options",List())
          
          jqp.plugins  must_== Nil
        }
        
        " correctly adds known plugins " in {
          val jqp = new JqPlot(0, 0, """{seriesDefaults: {renderer: jQuery.jqplot.PieRenderer,rendererOptions: {showDataLabels: true}},legend: { show:true, location: 'e' }}""",List())
